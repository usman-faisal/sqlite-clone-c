...
void create_new_root(Table *table, uint32_t right_child_page_num) {
    ...
    // copy data from root to left child
...
void update_internal_node_key(void* node, uint32_t old_max, uint32_t new_max) {
    uint32_t old_child_index = internal_node_find_child(node, old_max);
    *internal_node_key(node, old_child_index) = new_max;
}
...
void leaf_node_split_and_insert(Cursor *cursor, uint32_t key, Row *value) {
    ...
    *leaf_node_next_leaf(old_node) = new_page_num;
    ...
    // moving elements to their corresponding nodes
    for (int32_t i = LEAF_NODE_MAX_CELLS; i >= 0; i--) {
        ...
        if (i == cursor->cell_num) {
            ...
            *leaf_node_key(destination_node, index_within_node) = key;
            ...
            // before we add the new cell we put the old cells in
            // original place in the destination_node
...
Cursor* internal_node_find(Table* table, uint32_t page_num, uint32_t key) {
    uint32_t *node = get_page(table->pager, page_num);
